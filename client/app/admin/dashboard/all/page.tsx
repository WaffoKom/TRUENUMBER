...
export default function AllUsersGameHistory() {
  ...
  useEffect(() => {
    const fetchAllHistory = async () => {
      try {
        const { data } = await apiClient.get("/history/all"); // Adapter selon la route exacte
        setHistory(data);
        ...
        alert(
          error?.response?.data?.message ||
            "Erreur lors du chargement de l'historiqu"
        );
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="p-4 max-w-5xl mx-auto">
      <h1 className="text-2xl font-bold mb-4 text-center">
        Historique global des parties (Tous les utilisateurs)
      </h1>

      {loading ? (
        <p className="text-center">Chargement...</p>
      ) : history.length === 0 ? (
        <p className="text-center">Aucune partie trouvée.</p>
      ) : (
        <>
            ...
            <table className="w-full border-collapse border border-gray-300 text-sm">
              <thead>
                <tr className="bg-gray-100 text-left">
                  ...
                  <th className="border px-2 py-1">ID Partie</th>
              ...
              <tbody>
                {paginatedHistory.map((entry) => (
                  <tr key={entry._id} className="hover:bg-gray-50">
                    ...
                    <td className="border px-2 py-1">{entry.gameId}</td>
                    <td className="border px-2 py-1">
